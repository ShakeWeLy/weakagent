...
class AgentManager:
    """Manage agent lifecycle, registry, and parent/child relationships.

    This is a singleton - use AgentManager.instance() to get the global instance.
    """
    ...
    def __init__(self, factory: Optional[AgentFactory] = None):
        if AgentManager._instance is not None:
            raise RuntimeError(
                "AgentManager is a singleton. Use AgentManager.instance() instead."
            )
        self.factory = factory or AgentFactory()
        self._agents: Dict[str, ManagedAgent] = {}

    @classmethod
    async def instance(cls, factory: Optional[AgentFactory] = None) -> "AgentManager":
    ...
    def register(
        ...
        *,
        ...
        # Auto-wire manager context for multi-agent capable implementations.
        # Uses the singleton instance reference.
        if hasattr(agent, "agent_manager"):
            setattr(agent, "agent_manager", AgentManager._instance)
    ...
    def get_agent_summaries(self) -> Dict[str, Dict[str, str]]:
        ...
        Returns:
            Dict mapping agent_id to {name, description}:
            {
                "agent_xxx": {"name": "chat", "description": "..."},
                "agent_yyy": {"name": "toolcall", "description": "..."},
            }
        """
        return {
            agent_id: {
